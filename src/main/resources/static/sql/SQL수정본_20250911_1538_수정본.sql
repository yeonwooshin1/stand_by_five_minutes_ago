...
CREATE TABLE  pjWorker  (
  pjRoleNo			int UNSIGNED NOT NULL PRIMARY KEY AUTO_INCREMENT,						-- pj역할No
  pjNo 				INT UNSIGNED NOT NULL,                					-- 프로젝트명
  userNo    		INT UNSIGNED NOT NULL,                					-- 사원 
  pjRoleName  	 		VARCHAR(120) NOT NULL,                          	-- 체크리스트 이름 -> 사본이라면 복사 시점에 넣어주기
  pjRoleDescription    	longText NULL,										-- 체크리스트 설명 -> 사본이라면 복사 시점에 넣어주기
  pjRoleLv				int not null default 5,								-- 1~5  >> 1 : 엘리트 2 : 베테랑 3 : 숙련자 4 : 초급자 5 : 신입생 
  pjRoleStatus			TINYINT NOT NULL DEFAULT 1,							-- 상태 		
...
) ENGINE=InnoDB AUTO_INCREMENT=7000001 ;


/* ========================================================= 
 * 3-3) PjChecklist table
 *  프로젝트 체크리스트 "사본" 헤더
 * 20250903_옹태경_협의결과 테이블 삭제
 * ========================================================= */
...
CREATE TABLE  pjPerform (
  ...
  pjRoleNo    		INT UNSIGNED NOT NULL,                         			-- [FK] 어떤역 할의 누구에게(프로젝트 멤버여야 함)
  
  -- 일정(서비스에서 알림 계산용): 4구간(시작전/시작후/종료전/종료후) 알림은 "테이블 추가 없이" 로직에서 처리 가능
  pfStart 	DATETIME NULL,                               	  		-- 시작 시각(없으면 당일/즉시형 등 정책)
  pfEnd   	DATETIME NULL,                               	  		-- 종료 시각
  ...
  CONSTRAINT
    FOREIGN KEY (pjRoleNo) REFERENCES pjWorker(pjRoleNo)
